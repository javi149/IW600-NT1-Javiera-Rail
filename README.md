...
Se creó el componente `TarjetaProducto.vue`. Este es el que recibe los datos (nombre, categoría, productor, comuna y precio) mediante props. La información se envía desde el componente padre que es `Productos.vue`, el cual almacena un arreglo con 6 productos correspondientes a 3 comunas de Ñuble que son Pinto, Chillán y San Carlos.

## Parte C - Listas y condicionales
Se implementó un filtro mediante una etiqueta `<select>` vinculada a `v-model`. Se utilizó una propiedad computada (computed) para generar dinámicamente la lista `productosFiltrados`, y la directiva `v-for` recorre esta lista computada. Se incorporó `v-if` para evaluar si el largo del arreglo filtrado es cero, en cuyo caso se muestra un mensaje de "no hay productos disponibles" (si se selecciona Bulnes) y un `v-else` para renderizar el catálogo cuando existen coincidencias.
...
Se implementó un diseño de interfaz minimalista y altamente estructurado, caracterizado por el uso de una paleta estricta de dos colores (azul marino profundo y fondo blanco). Se utilizaron bordes sólidos de 3px, esquinas sin redondear y tipografía sans-serif audaz. Esta arquitectura visual proporciona máxima legibilidad, coherencia absoluta entre las vistas y una estética moderna, cumpliendo con los requerimientos visuales solicitados.
